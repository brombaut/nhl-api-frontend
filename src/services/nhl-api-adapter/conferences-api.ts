...
class ConferencesApi extends Api {
  ...
  public getConferences(): Promise<Array<Conference>>{
    return this.get<NhlApiResponseConferences>(API_CONFERENCES)
      .then((conferencesApiResponse: AxiosResponse<NhlApiResponseConferences>) => {
        const { conferences: nhlApiConferences } = conferencesApiResponse.data;
        const conferences: Array<Conference> = nhlApiConferences.map((nhlApiConference: NhlApiConference) => new Conference(nhlApiConference));
        return conferences;
      })
